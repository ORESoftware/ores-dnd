...
void main() {
  ...
  testWidgets('Flutter draggable carries the shared JSON envelope', (
    tester,
  ) async {
  ...
  testWidgets(
    'a real drag onto an accepting target drives the shared state machine to dropped',
    (tester) async {
      ...
      await tester.pumpWidget(
        harness(
          ...
          policy: textPolicy,
          states: states,
          onAccepted: (envelope, result) async => accepted.add(
            '${envelope.dragId}:${result.operation?.wire}:${result.targetId}',
          ),
        ),
      );
      ...
      expect(otel.events.map((e) => e.phase), [
      ...
      expect(
        jsonEncode(
          otel.events.map((e) => e.toJson()).toList(),
        ).contains('hello'),
        isFalse,
      );
    },
  ...
  testWidgets('a rejecting target reports the reason and never accepts', (
    tester,
  ) async {
    final controller = OresDndController();
    final accepted = <String>[];
  ...
  testWidgets(
    'leaving the target returns to dragging; releasing outside cancels',
    (tester) async {
      final controller = OresDndController();
      await tester.pumpWidget(
        harness(
    ...
    expect(
      zoneStateFor(
        const DndSessionSnapshot(
          ...
          targetId: 'a',
          errorCode: DndRejectCode.tooManyItems,
        ),
        'a',
      ),
      OresZoneState.rejecting,
    );
    expect(
      zoneStateFor(
        const DndSessionSnapshot(state: DndSessionState.dropped, targetId: 'a'),
        'a',
      ),
      OresZoneState.dropped,
    );
