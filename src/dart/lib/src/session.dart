...
enum DndSessionState {
  ...
  static DndSessionState parse(Object? value) =>
      DndSessionState.values.firstWhere(
        (candidate) => candidate.wire == value,
        orElse: () =>
            throw FormatException('unsupported session state: $value'),
      );
}
...
enum DndSessionInputKind {
  ...
  static DndSessionInputKind parse(Object? value) =>
      DndSessionInputKind.values.firstWhere(
        (candidate) => candidate.wire == value,
        orElse: () =>
            throw FormatException('unsupported session input kind: $value'),
      );
}

final class DndSessionInput {
  const DndSessionInput({
    required this.kind,
    this.envelope,
    this.targetId,
    this.policy,
    this.preferredOperation,
  });
  ...
  factory DndSessionInput.fromJson(Map<String, Object?> json) {
    ...
    return DndSessionInput(
      ...
      envelope: envelope == null
          ? null
          : DndEnvelope.fromJson(
              (envelope as Map).cast<String, Object?>(),
              structural: true,
            ),
...
final class DndSessionSnapshot {
  ...
  @override
  int get hashCode =>
      Object.hash(state, dragId, targetId, operation, errorCode);
...
final class DndSessionTrace {
  ...
  factory DndSessionTrace.fromJson(Map<String, Object?> json) {
    ...
    if (description != null &&
        Wire.codePoints(description) > Wire.traceDescriptionMax) {
    ...
    if (trace.inputs.length != trace.expected.length) {
      throw const FormatException(
        'trace inputs and expected must have the same length',
      );
    }
    return trace;
  }
...
final class DndSession {
  ...
  DndSessionSnapshot _next(DndSessionInput input) {
    ...
    if (input.kind == DndSessionInputKind.start) {
      ...
      if (candidate != null) {
        try {
          valid = DndEnvelope.fromJson(
            candidate.toJson(),
            maxItems: _codec.maxItems,
          );
        } on FormatException {
          valid = null;
        }
      ...
      return valid == null
          ? const DndSessionSnapshot(
              ...
              errorCode: DndRejectCode.invalidEnvelope,
            )
          : DndSessionSnapshot(
...
final class TraceDivergence {
  ...
  @override
  String toString() =>
      'trace $traceId diverged at step $step: expected $expected, got $actual';
}
...
TraceDivergence? replayTrace(DndSessionTrace trace) {
  if (trace.inputs.length != trace.expected.length) {
    return TraceDivergence(
      traceId: trace.id,
      step: trace.inputs.length < trace.expected.length
          ? trace.inputs.length
          : trace.expected.length,
    );
  }
