...
final class _Journal implements DndEffectJournalPort {
  ...
  @override
  Future<bool> hasCompleted(String idempotencyKey, DndEffectStage stage) async =>
      completed.contains(_key(idempotencyKey, stage));

  @override
  Future<void> markCompleted(String idempotencyKey, DndEffectStage stage) async {
...
final class _Otel implements OresOtelSupabasePort {
  ...
  @override
  Future<void> syncDndEventToSupabase(DndTelemetryEvent event, String idempotencyKey) async {
    expect(jsonEncode(event.toJson()).contains('hello'), isFalse);
...
void main() {
  ...
  test('RxDart effects share a stable idempotency key and redact payloads', () async {
    final calls = <String>[];
    final keys = <String>[];
    ...
    await bus.dispose();
    expect(calls, ['forms', 'opto-local', 'opto-supabase', 'otel-local', 'otel-supabase']);
    expect(keys.toSet(), {dndEffectKey(result)});
    expect(events.map((event) => event.status), everyElement(DndEffectStatus.completed));
    expect(jsonEncode(events.map((event) => event.toJson()).toList()).contains('hello'), isFalse);
  });

  test('retry skips completed stages after a later OTel Supabase failure', () async {
    final calls = <String>[];
    final keys = <String>[];
    final events = <DndEffectReceipt>[];
    final journal = _Journal();
    ...
    expect(
      ...
      ['forms:skipped', 'opto-local:skipped', 'opto-supabase:skipped', 'otel-local:skipped', 'otel-supabase:completed'],
    );
