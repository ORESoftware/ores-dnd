...
void main() {
  final traces = corpus()
      .where(
        (c) =>
            c.declaration == 'DndSessionTrace' && c.expectation == 'accepted',
      )
      .map(
        (c) => decodeDeclaration('DndSessionTrace', c.json) as DndSessionTrace,
      )
      .toList();

  test('every shared DndSessionTrace replays identically', () {
    expect(traces.length, greaterThanOrEqualTo(20));
    final divergences = traces
        .map(replayTrace)
        .whereType<TraceDivergence>()
        .map((d) => d.toString())
        .toList();
  ...
  test('session exposes envelope, result and listeners', () {
    final envelope = DndEnvelope.fromJson(
      readJson('instances/DndEnvelope/valid/text-copy.json'),
    );
    final session = DndSession();
    final seen = <String>[];
    final unsubscribe = session.subscribe(
      (snapshot, input) =>
          seen.add('${input.kind.wire}:${snapshot.state.wire}'),
    );
    ...
    session.apply(
      DndSessionInput.enter(
        const DndDropPolicy(
          targetId: 'zone-a',
          allowedOperations: [DndOperation.move],
  ...
  test('malformed inputs are ignored', () {
    ...
    expect(
      session.apply(
        const DndSessionInput(kind: DndSessionInputKind.enter, targetId: 'z'),
      ),
      before,
    );
    expect(
      session.apply(const DndSessionInput(kind: DndSessionInputKind.leave)),
      before,
    );
    expect(
      session.apply(
