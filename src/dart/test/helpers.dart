...
List<CorpusCase> corpus() {
  ...
  final decls = root.listSync().whereType<Directory>().toList()
    ..sort((a, b) => a.path.compareTo(b.path));
  for (final decl in decls) {
    final name = decl.uri.pathSegments.where((s) => s.isNotEmpty).last;
    for (final (lane, expectation) in const [
      ('valid', 'accepted'),
      ('invalid', 'rejected'),
    ]) {
...
Map<String, Object?> readJson(String rel) =>
    (jsonDecode(File('$contracts/$rel').readAsStringSync()) as Map)
        .cast<String, Object?>();
