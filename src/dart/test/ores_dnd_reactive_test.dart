...
void main() {
  test('RxDart bus keeps replayable state and telemetry payload-free', () async {
    ...
    bus.emit(DndLifecyclePhase.dragOver, envelope(), operation: DndOperation.copy, targetId: 'zone-a');
    bus.emit(DndLifecyclePhase.drop, envelope(), operation: DndOperation.copy, targetId: 'zone-a');
    bus.emit(DndLifecyclePhase.dragEnd, envelope(), operation: DndOperation.copy, targetId: 'zone-a');
    ...
    expect(jsonEncode(states.map((state) => state.toJson()).toList()), isNot(contains('TOP-SECRET-DRAG-DATA')));
    expect(jsonEncode(telemetry.map((event) => event.toJson()).toList()), isNot(contains('TOP-SECRET-DRAG-DATA')));
  ...
  test('strict lifecycle rejects out-of-order, cross-drag and post-drop phases', () async {
    final bus = OresDndReactiveBus();
    expect(() => bus.emit(DndLifecyclePhase.dragOver, envelope()), throwsFormatException);
    bus.emit(DndLifecyclePhase.dragStart, envelope());
    expect(() => bus.emit(DndLifecyclePhase.dragEnter, envelope('drag-other')), throwsFormatException);
    bus.emit(DndLifecyclePhase.drop, envelope(), operation: DndOperation.copy, targetId: 'zone-a');
    expect(() => bus.emit(DndLifecyclePhase.drop, envelope(), operation: DndOperation.copy), throwsFormatException);
    expect(() => bus.emit(DndLifecyclePhase.dragOver, envelope()), throwsFormatException);
    bus.emit(DndLifecyclePhase.dragEnd, envelope());
    expect(() => bus.emit(DndLifecyclePhase.dragEnd, envelope()), throwsFormatException);
    await bus.dispose();
  });

  test('reactive options reject source-disallowed operations and empty targets', () async {
    final bus = OresDndReactiveBus();
    bus.emit(DndLifecyclePhase.dragStart, envelope());
    expect(
      () => bus.emit(DndLifecyclePhase.dragOver, envelope(), operation: DndOperation.link),
  ...
  test('external compatible mode accepts one-shot drop without active replay state', () async {
    final guard = DndLifecycleGuard(mode: DndLifecycleMode.externalDropCompatible);
    guard.accept(DndLifecyclePhase.drop, 'external-1');
    expect(guard.active, isFalse);

    final bus = OresDndReactiveBus(lifecycleMode: DndLifecycleMode.externalDropCompatible);
    final states = <DndReactiveState>[];
