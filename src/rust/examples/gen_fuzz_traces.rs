...
fn main() {
    ...
    let seed_base: u64 = args.next().map(|v| v.parse().expect("seed")).unwrap_or(0x5eed_0000);
    std::fs::create_dir_all(&out).expect("mkdir");
    for i in 0..count {
        let seed = seed_base + u64::from(i);
        let inputs = random_sequence(seed, steps);
        let mut session = DndSession::new();
        let expected = inputs.iter().map(|input| session.apply(input).clone()).collect();
        ...
        trace.structural().expect("generated trace is structurally valid");
