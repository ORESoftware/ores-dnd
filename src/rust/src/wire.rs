...
pub fn is_safe_id(value: &str) -> bool {
    ...
    value.len() <= SAFE_ID_MAX && chars.all(|c| c.is_ascii_alphanumeric() || matches!(c, '.' | '_' | ':' | '-'))
}

pub fn check_safe_id(value: &str, label: &str) -> Result<(), DndError> {
    if is_safe_id(value) { Ok(()) } else { Err(err(label, "must match ^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$")) }
}
...
pub fn is_protocol_id(value: &str) -> bool {
    let Some(rest) = value.strip_prefix("ores.dnd/v") else { return false };
    let bytes = rest.as_bytes();
    !bytes.is_empty() && bytes.len() <= 3 && bytes[0].is_ascii_digit() && bytes[0] != b'0' && bytes.iter().all(u8::is_ascii_digit)
}
...
fn is_media_token(token: &str) -> bool {
    ...
    !bytes.is_empty()
        ...
        && bytes.iter().all(|b| b.is_ascii_lowercase() || b.is_ascii_digit() || matches!(b, b'!' | b'#' | b'$' | b'&' | b'^' | b'_' | b'.' | b'+' | b'-'))
}

/// `^[a-z0-9][a-z0-9!#$&^_.+-]{0,126}/[a-z0-9][a-z0-9!#$&^_.+-]{0,126}$` (3..=255 chars)
pub fn is_media_type(value: &str) -> bool {
    value.len() >= 3 && value.len() <= MEDIA_TYPE_MAX && value.split_once('/').is_some_and(|(t, s)| is_media_token(t) && is_media_token(s))
}
...
pub fn is_media_type_pattern(value: &str) -> bool {
    value.len() >= 3
        ...
        && value.split_once('/').is_some_and(|(t, s)| is_media_token(t) && (s == "*" || is_media_token(s)))
}
...
pub fn is_traceparent(value: &str) -> bool {
    ...
    parts.len() == 4
        && [2usize, 32, 16, 2].iter().zip(&parts).all(|(len, part)| part.len() == *len && part.bytes().all(|b| b.is_ascii_digit() || (b'a'..=b'f').contains(&b)))
}
...
pub fn check_len(len: usize, min: usize, max: usize, label: &str) -> Result<(), DndError> {
    if len < min || len > max { Err(err(label, &format!("must have between {min} and {max} entries"))) } else { Ok(()) }
}
...
mod tests {
    ...
    fn scalars_match_the_contract_patterns() {
        assert!(is_safe_id("drag-0001") && is_safe_id("A1.b_c:d-e") && is_safe_id(&"x".repeat(128)));
        assert!(!is_safe_id("") && !is_safe_id("-lead") && !is_safe_id("a b") && !is_safe_id(&"x".repeat(129)) && !is_safe_id("é"));
        assert!(is_protocol_id("ores.dnd/v1") && is_protocol_id("ores.dnd/v999"));
        assert!(!is_protocol_id("ores.dnd/v0") && !is_protocol_id("ores.dnd/v") && !is_protocol_id("ores.dnd/v1000") && !is_protocol_id("ORES.DND/v1"));
        assert!(is_media_type("text/plain") && is_media_type("application/vnd.ores.dnd+json"));
        assert!(!is_media_type("text") && !is_media_type("Text/Plain") && !is_media_type("text/plain; charset=utf-8") && !is_media_type("text/*"));
        assert!(is_media_type_pattern("text/*") && !is_media_type_pattern("*/*") && !is_media_type_pattern("text/"));
        assert!(is_traceparent("00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01"));
        assert!(!is_traceparent("00-4BF92F3577B34DA6A3CE929D0E0E4736-00f067aa0ba902b7-01") && !is_traceparent("abc"));
        assert!(is_error_code("http-500") && !is_error_code("Cancelled") && !is_error_code("a b") && !is_error_code(&"x".repeat(65)));
