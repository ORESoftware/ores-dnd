...
fn every_corpus_instance_gets_the_declared_verdict() {
    ...
    assert!(corpus.len() >= 70, "corpus unexpectedly small: {}", corpus.len());
    let mut failures = Vec::new();
    for (declaration, expectation, file, json) in &corpus {
        assert!(DECLARATIONS.contains(&declaration.as_str()), "unknown declaration dir {declaration}");
        let verdict = match decode_declaration(declaration, json) {
            Ok(()) => "accepted",
            Err(_) => "rejected",
        };
        if verdict != *expectation {
            failures.push(format!("{declaration}/{expectation}/{file}: rust said {verdict}"));
        }
    }
...
fn every_declaration_has_corpus_coverage() {
    ...
    for declaration in DECLARATIONS {
        assert!(corpus.iter().any(|(d, e, _, _)| d == declaration && *e == "accepted"), "{declaration} has no valid instance");
        assert!(corpus.iter().any(|(d, e, _, _)| d == declaration && *e == "rejected"), "{declaration} has no invalid instance");
    }
}
