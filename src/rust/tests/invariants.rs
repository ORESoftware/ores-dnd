...
fn check_invariants(seed: u64) {
    ...
    for (step, input) in inputs.iter().enumerate() {
        ...
        let ctx = || format!("seed {seed:#x} step {step} input {:?} -> {snapshot:?}", input.kind);
        // 1. every snapshot is structurally valid and round-trips through JSON
        snapshot.structural().unwrap_or_else(|e| panic!("{}: {e}", ctx()));
        let json = serde_json::to_string(&snapshot).unwrap();
        assert_eq!(serde_json::from_str::<DndSessionSnapshot>(&json).unwrap(), snapshot, "{}", ctx());
        ...
        match snapshot.state {
            DndSessionState::Idle => {
                assert!(snapshot.drag_id.is_none() && snapshot.target_id.is_none() && snapshot.operation.is_none(), "{}", ctx());
                assert!(session.envelope().is_none(), "{}", ctx());
            }
            DndSessionState::Dragging => {
                assert!(snapshot.drag_id.is_some() && snapshot.operation.is_none(), "{}", ctx());
                // 3. a rejecting target always carries its reason, and only then
                assert_eq!(snapshot.target_id.is_some(), snapshot.error_code.is_some(), "{}", ctx());
            }
            DndSessionState::OverTarget => {
                ...
                assert!(snapshot.target_id.is_some() && snapshot.error_code.is_none(), "{}", ctx());
                if input.kind == DndSessionInputKind::Enter && snapshot != previous {
                    ...
                    assert_eq!(snapshot.target_id.as_deref(), Some(policy.target_id.as_str()), "{}", ctx());
                }
            }
            DndSessionState::Dropped => {
                assert!(snapshot.operation.is_some() && snapshot.target_id.is_some() && snapshot.error_code.is_none(), "{}", ctx());
            ...
            DndSessionState::Cancelled => {
                assert!(snapshot.error_code.is_some() && snapshot.operation.is_none(), "{}", ctx());
                let result = snapshot.result().unwrap();
                assert!(!result.accepted, "{}", ctx());
                assert_eq!(result.error_code.as_deref(), snapshot.error_code.map(|c| c.wire()), "{}", ctx());
            }
        }
        // 6. dragId only changes on start
        if input.kind != DndSessionInputKind::Start && !previous.state.is_terminal() && previous.state != DndSessionState::Idle {
...
const XORSHIFT_SEED_7_FIRST_THREE: [u64; 3] = [0xd1fb_af7f_728d_2eae, 0xeda4_6c77_629d_a6ae, 0x16df_9d6a_c76b_d322];
