...
`contracts/` is the authority for API and data-shape semantics in `ORESoftware/ores-dnd`. `conformance/` is the shared behavioral corpus that runtime implementations, adapters, and compatibility layers must consume.
...
The initial `cases/bootstrap.v1.json` case establishes this repository-level authority boundary only. It does **not** prove domain-level runtime equivalence. Existing cross-runtime conformance machinery can move or reference its shared vectors here incrementally; do not duplicate case truth between `scripts/`, runtime trees, and this directory.
